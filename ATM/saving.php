<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    <style>
            body{
            background-image: url("money2.jpg");
            background-repeat: no-repeat;
            background-size:cover;
            background-position:initial;
            text-align: center;
          }
        
    </style>
</head>
<body>
   <div style="text-align: center;margin-left: 450px; margin-top:150px; background-color: rgb(40, 146, 150);width: 400px;border: solid 3px rgb(37, 37, 129);border-radius: 7px; height: 200px;">

       <h2> Saving account</h2>
     <a href="Withdraw.php"> <input type="submit" name="btn2" value="Withdraw"></a> 
     <a href="Deposite.php"> <input type="submit" name="btn2" value="Deposite"></a> 
     <a href="Checkblnc.php"> <input type="submit" name="btn2" value="Check Balance"></a> 
 </div>
</body>
</html>